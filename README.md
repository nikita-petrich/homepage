...
scripts/
  ...
  export-markdown.mjs           Whole site → one bilingual Markdown + images
  lib/png.mjs                   Minimal PNG reader shared by both
  lib/ts-loader.mjs             Lets plain node import lib/content/*.ts
export/                 Generated by `pnpm export` (Markdown + images)
```
...
```bash
pnpm export            # writes export/ (override with a path argument)
```

`scripts/export-markdown.mjs` writes the whole site into one folder that can be
zipped and sent on: `export/nikita-petrich-homepage.md` with the German and the
English version of every page one after the other — profile, key facts,
projects, testimonials (including each one's original wording), skills,
certificates with their syllabi, imprint and privacy policy — plus `images/`
holding every file from `public/assets`, referenced by relative path so the
document renders offline. Links point at the live site, since a Markdown file
that has left this repository cannot resolve `/de/projects/…` on its own.

The content comes from the same `getContent(locale)` the pages use, so an
export cannot claim something the site does not. The two legal pages are the
exception — they are JSX, and are converted from their source by a small
reader that covers the eight tags they use and throws on anything else, rather
than dropping a paragraph it did not understand.
